...
Le test \brique{touche un} est un peu particulier : il permet de détecter quand
notre objet touche une \important{copie} d'un autre objet. Il prend en
paramètre le \important{modèle} (l'objet dont on va faire des copies). Dans
notre script, le modèle est donc \brique{Fruits}.

Si on teste le script maintenant, il ne se passera rien : le contact entre la
bestiole et les fruits ne sera pas détecté. Le test ne fonctionne que sur des
\emph{copies} du modèle. Il faut donc faire une copie des fruits (icône
\icon{dupliquer}), et cette fois, le test va marcher. L'intérêt de ce test est
qu'on va pouvoir faire autant de copies de fruits qu'on veut, le test marchera
pour toutes ces copies.

De même, créons un script \code{estmangé} dans les fruits, inversé par rapport
au script \code{mange} :

\capture{57.png}

On peut maintenant tester notre système de nourriture. Démarrons les scripts
\code{mange} et \code{estmangé} de nos modèles de bestiole et de fruit, puis
créons des copies des modèles.

\astuce{Pour afficher la valeur d'une variable sur le bureau, il suffit de
l'attraper dans le visualisateur, et de la lâcher sur l'aire de jeu. Pratique
pour voir comment les programmes se comportent.}

Tu peux aussi démarrer les scripts \code{mouvementBestiole} et \code{fatigue}
de la copie de bestiole.

Lorsque notre copie de bestiole passe sur la copie de fruit, le niveau des
fruits baisse, et l'énergie de la bestiole augmente.

\capture{58.png}

\astuce{À toi de jouer~: essaye de modifier les fruits pour qu'ils affichent en
permanence leur niveau d'énergie restante. Petite piste : on peut créer un
objet texte qu'on \emph{ajoute} au fruit. Pour permettre à un fruit d'accepter
des \emph{objets inclus}, il faut activer l'option dans son menu (icône
\icon{menu} de son halo).}

Ça y est, notre simulation prend en compte l'alimentation de nos bestioles. Si
elles ne mangent pas, elles perdent de l'énergie et finissent par mourir.
...
Ce script utilise deux tests imbriqués pour déplacer le \important{curseur} du
conteneur (le rectangle noir) sur le bon dessin, en fonction du niveau
d'énergie.

Ensuite, il change \important{l'allure} des fruits (brique \brique{allure} dans
la catégorie \important{Allure} des fruits) en utilisant la brique
\brique{graphique sous curseur} de la catégorie \important{Conteneur} du
conteneur.

On peut ensuite démarrer le script (et le laisser tourner en permanence), puis
changer à la main la valeur de l'énergie des fruits dans leur visualisateur.
Dès qu'on passe en-dessous de 65 puis en-dessous de 33, les fruits devraient
changer d'apparence.
...
\astuce{À toi de jouer~: pour finir cette partie, modifie le script de déplacement des bestioles afin que leur niveau d'énergie influe sur leur vitesse : rapide quand elles sont pleines d'énergie, lentes quand elles sont épuisées.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
